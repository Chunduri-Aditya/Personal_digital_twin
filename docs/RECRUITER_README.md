...
**The active persona, Mara Ellison, is a synthetic example profile invented to exercise every feature —
nothing shown here is a real person's data.** The app also has a persona switcher (Status tab) with several
other fictional personas registered, and an "import a new persona from a .md file" control, if you want to see
that in action.
